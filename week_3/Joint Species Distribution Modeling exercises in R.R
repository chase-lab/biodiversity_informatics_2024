##################################################################
##     R Practical for MLU Block Course: Introduction           ##
##           to Biodiversity Informatics - Part II              ##
##                                                              ##
##                    Monday, May 8, 2023                       ##
...
##################################################################
##    Part I: Univariate Examples with Simulated Data          ##
##################################################################

# We will simulate data for a single covariate (i.e. predictor) x on n=50 sampling
# units (i.e. sites). We then will analyze this data with three different models: 
...
# an equation following the basic y=mx+b format where b (and beta 1) = intercept,
# m (and beta2) = slope, and y (and L) = our response variable).
L = beta1 + beta2*x
...
# the Hmsc function only constructs the model object. We have to fit the model in a separate
# step.

# Hmsc uses Bayesian inference, and while there are many conceptual and statistical details
...
# estimate as in the m.lm linear model we just ran, but rather a distribution (called the 
# posterior distribution), from which the real value of the parameter likely comes from.
...
# We have built our Hmsc model object, but now we have to do the MCMC sampling, which is
# the process of fitting the model and generating the posterior distribution..
...
# (also known as posterior means) derived from the posterior distributions,
# just like our linear model object m.lm contained the parameter estimates 
# when we used the lm function.
...
# The results are similar to those we obtained with our simple linear model.
# The posterior mean of the intercept is close to 0 and the estimate of the slope
...
# (and thus yields specific parameter estimates with confidence intervals), 
# while the Hmsc package applies a Bayesian framework (and thus yields posterior distributions
# for the parameters with credible intervals).

# For further clarification a 95% confidence interval is the range of values that you can
# be 95% certain contains the true mean of the response of interest.
# With a 95% credible interval, there is a 95% probability that the true value of the
# response of interest (which is unknown) lies within this interval, given the data we have.
...
# We will check for convergence quantitatively in two ways. First with effective sample sizes and 
# second with potential scale reduction factors.

# Generally, effective sample size (ESS) measures the amount that autocorrelation 
# (i.e. dependency among samples) increases uncertainty relative to an independent sample.
# In the context of MCMC sampling, ESS measures the effectiveness of a sample chain.
# For example, 1000 samples with an ESS of 200 has a higher effectivness (and reliability) than
# 2000 samples with an ESS of 100.
...
# When convergence occurs, the distribution of samples from each chain should be the same 
# regardless of the starting values of the chains. 
# One way to check this is to compare the distributions of each chain, when convergence has 
# occurred, they should all have the same mean.
# Note that the output we get with PSRFs though is not actually the raw value of the mean,
# but a scaled value to make different distributions comparable.

gelman.diag(mpost$Beta, multivariate = FALSE)$psrf

# Since for both parameters the psrfs are very close to 1, this means that the two chains 
# gave consistent results, which is what we want to see.
...
#       occurrence. So the RMSE value is telling us by how much the probability of 
...
# below code. We will not go into these in detail today, but for reference SR2 can be viewed as a
...
# represents the 95% credible interval of the model predictions. Note that many points fall
...
# introduced additional variation to our response not captured by our predictor variable?
...
# vary in space. In our modeling, we therefore need to account for spatial autocorrelation,
...
# Download the bird data (data.csv) and the grid data (grid_1000.csv) from the class github
...
# and some environmental predictors (Habitat, and different measures of temperature). 
# JunJul = average temperature in June and July
# DJF = average temperature in December, January, and February
# AprMay = average temperature in April and May
# Then we have data on many different bird species.
...
# These correspond to broadleaf forest, coniferous forest, open habitat, urban habitat, 
# and wetlands.

# April/May climate refers to mean spring temperature in April and May.
hist(XData$clim)
...
# to match the spatial coordinate with the proper route
...
# we build the model object. Here we specify a polynomial term for temperature,
...
# We are adding an additional argument here with initPar. While there is a more technical
...
# in the northern part of the country where Wetlands predominate. We can see the lower effective
# sample size reflected in this for Wetlands. 

# Evaluate convergence quantitatively by examining potential scale reduction factors.
gelman.diag(mpost$Beta, multivariate = FALSE)$psrf

# We see some values substantially above 1 here, which indicates a lack of convergence.
# Since this is just an example, and to avoid excessive computation time, we will visualize
# our results as if there was convergence. However, note that to fix this issue, we would 
# need to rerun the model with more samples and possible more chains.
